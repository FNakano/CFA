...
async def playNote (note, octave, val) :
    pn=diat[note]
    if (pn>0):
      beeper = PWM(Pin(27, Pin.OUT), freq=int(pow2[octave]*baseFreq*pn), duty=512)
   # await asyncio.sleep(pow2[val]/64.0) # synchronous programming
    await asyncio.sleep(pow2[val]/64.0) # synchronous programming
    if (pn>0):
      beeper.deinit()
...
async def playFig (figure, note, octave=lastoctave) :
    pn=diat[note]
    print (pn)
    if (pn>0):
      beeper = PWM(Pin(27, Pin.OUT), freq=int(pow2[octave]*baseFreq*pn), duty=512)
    #await asyncio.sleep(pow2[fig[figure]]/64.0) # synchronous programming
    await asyncio.sleep(pow2[fig[figure]]/64.0) # synchronous programming
    if (pn>0):
      beeper.deinit()
    lastoctave=octave

async def playAquarela() :
...
async def Aplaynote():
    # sim! isto toca uma nota!
    await playFig('breve', 'a')
