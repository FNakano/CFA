...
class WATCH:
  ...
  def drawRing (self, size, value, text):
    ...
    arc.set_style_arc_color(lv.color_hex(0x0),lv.PART.KNOB)
  ...
  def drawHourMin (self):
    datetime=self.hwrtc.datetime()
    hours=datetime[4]
    mins=datetime[5]
    self.drawRing (180, mins, str(mins))
    self.drawNeedleAndRing (100, (hours%12)*5, str(hours))
  ...
  def sleepOnTimer (self):
    ...
    self.tim=lv.timer_create(lambda event: self.sleepNow(event), 120000, None) # https://forum.lvgl.io/t/how-to-delete-one-lv-timer-in-micropython-lgvl-version-8-3/9428/1
